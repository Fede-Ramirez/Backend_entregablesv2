...
describe("Tests server products", () => {
  ...
  it("post product", async () => {
    ...
    const response = await request(app).post("/api/products/new-products").send(product);
  ...
  it("get all products", async () => {
    ...
    const response = await request(app).get("/api/products/available-products").expect(200);
  ...
  it("update product", async () => {
    ...
    expect(response.body.name).toBe(productUpdated.name);

    expect(response.body.price).toBe(productUpdated.price);

    expect(response.body.stock).toBe(productUpdated.stock);

    expect(response.body.codebar).toBe(productUpdated.codebar);
  ...
  it("delete product", async () => {
    ...
    const saveProduct = await ProductsModel.find({ id: 5 });

    expect(saveProduct).toHaveLength(0);
